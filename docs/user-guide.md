...
Показывает историю запросов к выбранной таблице из `system.query_log`.

**Фильтры** (над таблицей):

- **User** — кнопки-переключатели по пользователям. Синяя = показывать, серая = скрыть
- **Kind** — аналогично по типу запроса (Select, Insert и т.д.)
...
**Кнопка `</>`** — показывает SQL-запрос, которым приложение получает историю. Можно скопировать и выполнить в ClickHouse-клиенте.
